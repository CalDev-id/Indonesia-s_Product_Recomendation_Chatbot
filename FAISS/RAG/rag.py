...
class Rag:
    ...
    def sort_resources(self, query, resources):
        resource_embeddings = self.embedding_model.encode([r['name'] for r in resources])
        query_embedding = self.embedding_model.encode([query])
    ...
    def rag_search(self, query):
        ...
        system_prompt = f"Anda adalah asisten pencarian barang yang hilang berbahasa indonesia. Berikut adalah detail barang yang paling mirip:\n{best_resource['nama_barang'], best_resource['deskripsi'], best_resource['lokasi_ditemukan'], best_resource['tanggal_ditemukan']}"
